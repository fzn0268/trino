 ...
 */
package io.prestosql.plugin.phoenix5;
...
import static io.prestosql.plugin.phoenix5.PhoenixColumnProperties.isPrimaryKey;
        ...
        implements ConnectorMetadata
{
    ...
    public JdbcTableHandle getTableHandle(ConnectorSession session, SchemaTableName schemaTableName)
    {
        return phoenixClient.getTableHandle(JdbcIdentity.from(session), schemaTableName)
                .map(tableHandle -> new JdbcTableHandle(
                        ...
                        MetadataUtil.toPrestoSchemaName(Optional.ofNullable(tableHandle.getSchemaName())).orElse(null),
    ...
    {
        JdbcTableHandle handle = (JdbcTableHandle) tableHandle;
        phoenixClient.execute(session, String.format(
                "ALTER TABLE %s ADD %s %s",
                MetadataUtil.getEscapedTableName(Optional.ofNullable(handle.getSchemaName()), handle.getTableName()),
                column.getName(),
    ...
    public void dropTable(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        if (hasRowkey) {
            ...
            phoenixClient.execute(session, String.format("DROP SEQUENCE %s", MetadataUtil.getEscapedTableName(Optional.ofNullable(jdbcHandle.getSchemaName()), jdbcHandle.getTableName() + "_sequence")));
        }
    ...
    public JdbcOutputTableHandle createTable(ConnectorSession session, ConnectorTableMetadata tableMetadata)
    {
        ...
        try (Connection connection = phoenixClient.getConnection(JdbcIdentity.from(session))) {
            ...
            if (rowkeyColumns.isEmpty()) {
                ...
                pkNames.add(ROWKEY);
                phoenixClient.execute(session, String.format("CREATE SEQUENCE %s", MetadataUtil.getEscapedTableName(schema, table + "_sequence")));
                hasUUIDRowkey = true;
            }
            ...
            String sql = String.format(
                    ...
                    immutable,
                    MetadataUtil.getEscapedTableName(schema, table),
